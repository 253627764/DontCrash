...
local MainGameScene =class("MainGameScene",function() return cc.Scene:create() end)
...
function MainGameScene:addButton(layer)
    ...
    retry:setScale(0.8)
    local menu = cc.Menu:create(retry,more)
...
--    end
    ...
    local more_callback = function()
        ...
        chuangkou=cc.Sprite:create("chuangkou2.png")
        chuangkou:setPosition(cc.p(self.size.width/2,self.size.height/2))
        chuangkou:setTag(10)
        layer:addChild(chuangkou,99)
        chuangkou:runAction(cc.Sequence:create(cc.ScaleTo:create(0.05,1.3),cc.ScaleTo:create(0.05,0.8)))
    ...
    local function onTouchBegan()
        ...
        chuangkou:runAction(cc.FadeOut:create(2.0))
    ...
    eventDispatcher:addEventListenerWithSceneGraphPriority(touch_listener,more)
...
function MainGameScene:switch(car,tm1,tm2,tm3,tm4,degree1,degree2,circleCenter1,circleCenter2,point1,point2)
    ...
    --创建第一个动作
    local circle1 = ss.CircleBy:create(tm1,circleCenter1,degree1)
    ...
    --右圆周运动
    local circle2 = ss.CircleBy:create(tm3,circleCenter2,degree2)
...
function MainGameScene:mainLogic(layer)
    ...
    cclog("----------width:%d",self.track_size.width)
    cclog("----------height:%d",self.track_size.height)
...
--        self.track_size.height=track:getContentSize().height*heightRate/widthRate
--    else
--        self.track_size.width=track:getContentSize().width*widthRate/heightRate
--       
--    end 
    --红车
    local car_red = cc.Sprite:create("car5.png")
    ...
    --黄车
    local car_yello = cc.Sprite:create("car3.png")
...
function MainGameScene:runRedCar(car)
    ...
    --创建动作
    local circle1 = ss.CircleBy:create(tm,{x=0,y=y1},180)
    local spawn1 = cc.Spawn:create(circle1,cc.RotateBy:create(tm,180))
    --右圆周运动
    local circle2 = ss.CircleBy:create(tm,{x=0,y=y2},180)
...
function MainGameScene:runYelloCar(car)
    ...
    if r > 50 and (self.yello_switch == false) then
        --变轨固定在这个点 进入内部的轨道
        local circle1 = cc.Spawn:create(ss.CircleBy:create(tm/2,{x=-30,y=self.track_size.height*0.4},-90),cc.RotateBy:create(tm/2,-90))
        local circle2 = cc.Spawn:create(ss.CircleBy:create(tm/2,{x=-162,y=self.track_size.height*0.0015},-90),cc.RotateBy:create(tm/2,-90))
    ...
    if r > 50 and (self.yello_switch == true) then
        --变轨固定在这个点 转到外部的轨道
        local circle1 = cc.Spawn:create(ss.CircleBy:create(tm/2,{x=-25,y=-self.track_size.height*0.4},-90),cc.RotateBy:create(tm/2,-90))
        local circle2 = cc.Spawn:create(ss.CircleBy:create(tm/2,{x=225,y=self.track_size.height*0.02},-90),cc.RotateBy:create(tm/2,-90))
